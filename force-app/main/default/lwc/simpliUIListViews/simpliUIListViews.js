...
export default class simpliUIListViews extends NavigationMixin(LightningElement) {
    ...
    @api listViewList               = [];          //holds the set of list views for the chosen object
    ...
    async getListViewActions() {
        ...
        if (this.virtual) {
            ...
            await getListViewActions({ objectType: this.selectedObject, listViewName: this.selectedListView, componentName: this.pageName }).then(result => {
                ...
                this.objectActionList = undefined;
                this.spinnerOff('getListViewActions');
    ...
    handleListViewActions(numSelectedRecords) {
        ...
        console.log('Can display actions - ' + this.canDisplayActions + ' for ' + this.pageName)
        console.log('Action list size - ' + this.objectActionList.length + ' for ' + this.pageName)
        console.log('Displayed action list size - ' + this.displayedActionList.length + ' for ' + this.pageName)
    }
    ...
    async getObjectsList() {
        if (this.objectList === undefined) //only get the list views if we have not retrieved them before
        {
            ...
            await getListViewObjects({ includedObjects: this.includedObjects, excludedObjects: this.excludedObjects }).then(result => {
    ...
    async getListViewsForObject() {
        if ((this.listViewListObject !== this.selectedObject || this.listviewOffset !== 0) && this.typeAheadListSearch === false) //only get the list views if its for a new object OR we are getting the next batch and we are not doing type ahead
        {
            ...
            if (this.pinnedListView !== undefined && this.firstListViewGet === true) {
                ...
                await getListViewId({ objectName: this.selectedObject, listViewName: this.pinnedListView }).then(result => {
        ...
        if (this.receivedMessage.type === 'selectrecordupdate') {
            ...
            if (this.selectedObject !== undefined
                ...
                if (!this.virtual) {
                    ...
                    isValidListViewDataRequest({ objectName: this.selectedObject, joinFieldName: this.joinFieldName, joinData: this.joinData }).then(result => {
    ...
    handleProcessListViewsButtonClick(event) {
        ...
        //if we have selected ALL list views to update
            ...
            updateAllListViews().then(result => {
                if (result === 'failed') {
    ...
    saveColumnWidth(newWidth, columnIndex) {
        ...
        updateUserConfigListViewWidth({ compName: this.pageName, configName: configName, columnIndex: columnIndex, width: newWidth }).then(() => { }).catch(error => {
                ...
                if (this.virtual) {
                    ...
                    updateRecord({ rowId: rowId, rowData: rowDataStr }).then(result => {
                        ...
                        if (result === '') {
                            ...
                            this.dispatchEvent(SLVHelper.createToast('error', '', 'Error', result, false));
                            this.spinnerOff('handleRowDataSave');
                        }
                    }).catch(error => {
        ...
        if (rowData !== undefined) {
            if (rowData.size > 0) {
                ...
                if (this.virtual) {
                    ...
                    updateRecords({ rowData: rowDataStr }).then(result => {
    ...
    displayHoverDetails(event) {
        try {
            ...
            if (this.displayRecordPopovers === true && this.hoverSFDCId !== sfdcId) {
                this.hoverSFDCId = sfdcId;
                this.hoverAPIName = apiName;
                this.hoverLabelName = labelName;
                this.hoverIsDisplayed = true;
                this.hoverPositionTop = clientY - 170;
                this.hoverPositionLeft = clientX - 15;
            }
        } catch (error) {
            SLVHelper.showErrorMessage(error);
        }
